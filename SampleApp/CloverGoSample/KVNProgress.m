...
#define readerImageHeightAdjust 45
...
static CGFloat const KVNContentViewCornerRadius = 4.0f;
...
@property (nonatomic) M13ProgressViewImage *readerResetProgressView;
...
- (void)setupStatus:(NSString *)status
{
	...
	if(![self.statusLabel.text isEqualToString:status])		// avoid double speak for ADA Voiceover
...
- (void)setupCancelButton:(BOOL)show{
    if (self.cancelButton==nil) {
        self.cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height-49, [UIScreen mainScreen].bounds.size.width, 49)];
    ...
    UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(cancelClicked:)];
...
- (void)cancelClicked:(id)sender{
    [[NSNotificationCenter defaultCenter] postNotificationName:CancelButtonClickedOnProgressWindowEventNotification object:self];
    self.cancelButton.hidden = YES;
}

+(void)showCancelButton:(BOOL)showOrHide{
    [[self sharedView] setupCancelButton:showOrHide];
}
...
+(void)initReaderReset{
    [[KVNProgress sharedView]initiateReaderReset];
}

- (void)initiateReaderReset{
    if (self.readerResetProgressView == nil) {
        self.readerResetProgressView = [[M13ProgressViewImage alloc] initWithFrame:CGRectMake(0, 0, self.configuration.circleSize, self.configuration.circleSize)];
        self.readerResetProgressView.center = self.center;
        CGRect midFrame = self.readerResetProgressView.frame;
        midFrame.origin.y = midFrame.origin.y - readerImageHeightAdjust;
        midFrame.origin.x = midFrame.origin.x - readerImageHeightAdjust/2;
        midFrame.size.height = midFrame.size.width = midFrame.size.width + readerImageHeightAdjust;
        self.readerResetProgressView.frame = midFrame;
        self.circleProgressView.hidden = YES;
        [self addSubview:self.readerResetProgressView];
    }
    [self.readerResetProgressView setProgressImage:[UIImage imageNamed:@"card-reader-100"]];
    [self addShimmerToCurrentView];
}

+(void)updateReaderResetProgress:(CGFloat)progress{
    [[KVNProgress sharedView]updateResetProgress:progress];
}

- (void)updateResetProgress:(CGFloat)progress{
    [self.readerResetProgressView setProgress:progress animated:YES];
}


+(void)completeReaderReset{
    [[KVNProgress sharedView]finishReaderUpdate];
    
}

- (void)finishReaderUpdate{
    [self.readerResetProgressView removeFromSuperview];
    self.shimmeringView.shimmering = NO;
    self.logoLabel.text = @"";
    self.readerResetProgressView = nil;self.shimmeringView=nil;self.logoLabel = nil;
    self.circleProgressView.hidden = NO;
}

- (void)addShimmerToCurrentView{
    
    if (self.shimmeringView == nil)
        self.shimmeringView = [[FBShimmeringView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-150, self.bounds.size.width, 150)];
    
    if (self.logoLabel == nil)
        self.logoLabel = [[UILabel alloc] initWithFrame:self.shimmeringView.bounds];
    
    self.shimmeringView.shimmering = YES;
    self.shimmeringView.shimmeringBeginFadeDuration = 0.15;
    self.shimmeringView.shimmeringOpacity = 0.0;
    [self addSubview:self.shimmeringView];
    
    self.logoLabel.text = @"Resetting Reader";
    self.logoLabel.font = [UIFont fontWithName:@"HelveticaNeue-UltraLight" size:40.0];
    ...
    self.shimmeringView.contentView = self.logoLabel;
}
